...
assert_contains "$tmpdir/render.out" "nft add element inet lan_proxy_route clients_v4 { 192.168.50.10 }"
...
LPR_CONFIG="$tmpdir/lan_proxy_route" LPR_COMMAND_RUNNER="$tmpdir/runner" LPR_EXEC_LOG="$exec_log" \
	LPR_DNSMASQ_CONF="$dns_conf" sh "$svc" apply
...
LPR_RPCD_SERVICE="$svc" LPR_CONFIG="$tmpdir/lan_proxy_route" PATH="$tmpdir:$PATH" LPR_IP_LOG="$tmpdir/ip.log" \
	sh "$rpc" call test_route 8.8.8.8 > "$tmpdir/rpc-route.json"
